...
class TaskService:
    ...
    def print_all_tasks(self, indent: int = 0) -> None:
        ...
        tab_indent = '\t' * indent
        print(f"{tab_indent}{'task name':^{max_name_length}} \t {'task description':^{max_description_length}} \t {'task status':^10} \t {'task due date':^{max_due_date_length}} \t {'task project id':^15}")
        print()
    ...
    def print_task(self, task: Type[Task], indent: int = 0) -> None:
        ...
        print(f"{tab_indent}{display_name:<{max_name_length}} \t {display_description:<{max_description_length}} \t {task.status:<10} \t {display_due_date:<{max_due_date_length}} \t {task.project_id:<15}")
